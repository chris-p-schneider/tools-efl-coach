...
def defineGrid (longestAndTotal, x, y):
	...
	if longestAndTotal[0] > (x or y):
		pass # will need to add error stuff in flask later
		# sys.exit ("Your longest word won't fit in this Wordsearch!")
	elif (x*y) > (longestAndTotal[1]*2):
		pass
	else:
		pass
		# print ("The ratio of word characters to grid coordinates is: " + str(longestAndTotal[1]) + ":" + str(x*y))
...
def defineCoords (coordsDictDefault, x, y):
	...
	while defineCoordsCounter < ((x*y)+1):
		...
		while foundCoord == False:
			...
			temporaryDict["Character"] = '–' #en dash
			temporaryDict["nType"] = '–'
			if xCounter < (x+1):
...
def makeCharArray (coordsDict, x, y):
	...
	while charCounter < ((x*y)+1):
		...
		charRowCounter += 1
	'''
	for i in range(1, len(coordsDict)):
		if i < ((x*charRowCounter)+1):
			charRowList.append(coordsDict[i]["Character"])
		else:
			charArrayList.append(charRowList)
			charRowList = []
			charRowCounter += 1
...
def fillSpace (coordsDict, x, y):
	fillSpaceTxt = open ('/home/chsch/Documents/PYTHON/Tools.EFL.Coach/TEFLC v0.0.1/TEFLC/static/py/fillSpace.txt', 'r')
...
# MAIN EXECUTION ////////////////////////////////////////////////////////
def generateWordSearchOriginal (vocabList, x, y): 
	...
	answerKeyList = answerKey.rsplit('\n')
	...
	wordsearchDict = {
		"x": x,
		"y": y,
		"longestAndTotal": longestAndTotal,
		"vocabList": vocabList,
		"answerKey": answerKeyList,
		"wordsearch": wordSearchList
	}

	return wordsearchDict


# wordSearchList = generateWordSearch (vocabList, x, y)

# -------------------------------------
# There were problems with large grids, idk why
# So I redid stuff:
...
def generateWordSearch (vocabList, x, y): 
	...
	# for key in coordsDict:
		# print(coordsDict[key]["Character"])
	...
	# print("charArrayList")
	# print(charArrayList)
	# print(charArrayList[0][0])
	# print("----------")

	answerKeyList = []
	for row in charArrayList:
		tempString = ""
		tempString = " ".join(row)
		answerKeyList.append(tempString)
	...
	fillSpaceTxt = open ('/home/chsch/Documents/CODE/Tools.EFL.Coach/TEFLC v0.0.1/TEFLC/static/py/fillSpace.txt', 'r')
	fillSpaceChars = fillSpaceTxt.read()

	for row in answerKeyList:
		tempString = ""
		for c in row:
			if c == "–":
				randomCharIndex = random.randint(0,993)
				randomChar = fillSpaceChars[randomCharIndex]
				tempString = tempString + randomChar
			else: 
				tempString = tempString + c 
		wordSearchList.append(tempString)

	# print("wordSearchList")	
	# print(wordSearchList)	
	# print("----------")
